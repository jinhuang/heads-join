...
//        logger.setLevel(Level.DEBUG);
        ...
        for (int i = 0; i < numCombination; i++) {
            ...
            combination = FormatUtils.getNthSubArray(combinationError,
            ...
            ids = FormatUtils.getSubArray(combination, 0, numVector - 1, ids);
            for (int j = 0; j < ids.length; j++) {
                idsLong[j] = Math.round(ids[j]);
            }
            String combinationId = FormatUtils.formatCombination(idsLong);
...
//					System.out.println("dual " + k + " : " + dualLower);
                ...
                for (int j = 0; j < numVector; j++) {
                    recordInSpace = FormatUtils.getNthSubArray(record, 2, j,
                            recordInSpace);
                    errorInSpace = FormatUtils.getNthSubArray(recordError,
                            recordErrorLength, j, errorInSpace);
                    long gridId = Math.round(ids[j]);
                    gridBound = grids[j].getGrid((int) gridId, gridBound);
                    gridError = FormatUtils.getSubArray(combination, offset + j
				 ...
				 * then rubner emd
				 */
                double[] rubnerValue = DistanceUtils.getRubnerValue(hist, dimension, bins);
                double rubnerEmd = DistanceUtils.getRubnerBound(rubnerValue, FormatUtils.toDoubleArray(rubnerKeys.get(combinationId)), dimension);
                maxLower = maxLower > rubnerEmd ? maxLower : rubnerEmd;

                double[] gridFlows = FormatUtils.getSubArray(combination,
                        numVector * errorLength + 1 + numVector,
                        combination.length - 1);
                for (int j = 0; j < numReference; j++) {
                    double upper = gridFlows[j]
                            + HistUtils.getFlowBetween(hist, references.get(j),
                            bins, dimension);
...
//			long tmpTimer = System.nanoTime();
            ...
            if (!recordCombination.equals(combinationId)) {
                ...
                assert(keys != null && keys.length == duals.length * 2);

                for (int k = 0; k < duals.length; k++) {
                    double dualLower = duals[k].getCKey(hist) + keys[k];
                    maxLower = dualLower > maxLower ? dualLower : maxLower;
                }
//                System.out.println("HEADS DEBUG: maxLower emdbr " + maxLower);
...
//               System.out.println("HEADS DEBUG: maxLower " + maxLower);
...
//                        TimerUtil.eliminatedCounter += counterCombination;
    ...
    public double[] getProjectionDistanceInGrid(double[] point) {
        ...
        distance[0] = getDistance(FormatUtils.getSubArray(projections, 0, 1),
                pointSouthern);
        distance[1] = getDistance(FormatUtils.getSubArray(projections, 2, 3),
                pointSouthern);
        return distance;
    }
...
//		}
    ...
    public double getEmdBr(double[] record, double[] recordError,
        ...
        // 3));
        double[] normalUpper = getNormal(FormatUtils
                .getSubArray(gridBound, 2, 3));
        double[] normalLower = getNormal(FormatUtils
        ...
        double[] lineNE = getLine(FormatUtils.getSubArray(gridBound, 2, 3),
                slopes[0]);
        double[] lineNW = getLine(FormatUtils.getSubArray(gridBound, 2, 3),
                slopes[1]);
        double[] lineSE = getLine(FormatUtils.getSubArray(gridBound, 0, 1),
                slopes[1]);
        double[] lineSW = getLine(FormatUtils.getSubArray(gridBound, 0, 1),
                slopes[0]);
...
//			dist += getEmdBrError(normalRecord, recordError, normalLower, gridError, numInterval, true);
    ...
    private double getEmdBrNormal(double[] record, double[] grid) {
        double dist = 0.0;
        double intersection = HistUtils.getNormalCDFIntersection(record, grid);
//		double min = (-1) * t[1];
//		double max = (-1) * t[0];
        double min = t[0];
        double max = t[1];
        ...
        if ((min - intersection) > 0 || (intersection - max) > 0
            ...
            dist = HistUtils.getCDFDifferenceBetweenNormals(record, grid, min,
                    max);
        }
            ...
            dist = HistUtils.getCDFDifferenceBetweenNormals(record, grid, min,
                    intersection)
                    + HistUtils.getCDFDifferenceBetweenNormals(record, grid,
                    intersection, max);
        }
    ...
    private double getEmdBrError(double[] dominated, double[] dominatedError,
                                 ...
                                 boolean partial) {
        double error = 0.0;
        double intersection = HistUtils.getNormalCDFIntersection(dominated,
                dominator);
        if (!partial) {
