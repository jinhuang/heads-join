...
public class FormatUtils {
    ...
    public static double[] getNormalizedDoubleArray(Text text, int start, int end) {
        ...
        return HistUtils.normalizeArray(array);
    }
...
//        }
    ...
    public static String formatCombination(double[] ids) {
        String idString = "";
        for (double id : ids) {
            idString += Math.round(id) + " ";
        }
        return idString.trim();
    }
...
//    }
    ...
    public static double[] subtractAvg(double[] array) {
    ...
    public static String formatPivots(double[][] pivots ) {
        StringBuilder builder = new StringBuilder();
        for(int i = 0; i < pivots.length; i++) {
            double[] each = pivots[i];
            for (int j = 0; j < each.length; j++) {
                builder.append(each[j]).append(" ");
            }
        }
        return builder.toString().trim();
    }

    public static double[][] getPivots(String string, int numBin) {
        String[] ary = string.split(" ");
        int numPivots = ary.length / numBin;
        double[][] pivots = new double[numPivots][numBin];
        for (int i = 0; i < numPivots; i++) {
            for (int j = 0; j < numBin; j++) {
                pivots[i][j] = Double.valueOf(ary[i * numBin + j]);
            }
        }
        return pivots;
    }
