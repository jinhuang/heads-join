...
public class DualBound implements Serializable {
    ...
    public DualBound(String value) {
        this(FormatUtils.toDoubleArray(value));
    }
...
//		}
    ...
    public String toString() {
        String text = FormatUtils.toTextString(solution);
        return text;
    }
    ...
    private double[] getOptimalFeasibleSolution(double[] histA,
        ...
        for (int i = 0; i < num; i++) {
            for (int j = 0; j < num; j++) {
                ...
                double dist = DistanceUtils.getGroundDist(DistanceUtils.getBin(bins, dimension, i),
                        DistanceUtils.getBin(bins, dimension, j), DistanceType.LTWO, null);
