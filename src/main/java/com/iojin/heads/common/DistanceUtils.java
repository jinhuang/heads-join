...
public class DistanceUtils {
    ...
    public static double get1dEmd(double[] histA, double[] histB, double[]bins) {
        double total = 0.0;
        TreeMap<Double, Double> hA = HistUtils.getDiscreteCDFNormalized(histA, bins);
        TreeMap<Double, Double> hB = HistUtils.getDiscreteCDFNormalized(histB, bins);
    ...
    public static double getEmd(double[] histA, double[] histB, int dimension, double[] bins, DistanceType distType, HashMap<String, Double> costMatrix) {
        if (HistUtils.sum(histA) != HistUtils.sum(histB)) {
            //System.out.println("Infeasible: " + FormatUtils.toString(histA) + " - " + FormatUtils.toString(histB));
            histA = HistUtils.normalizeArray(histA);
            histB = HistUtils.normalizeArray(histB);
        }
...
//			}
    ...
    public static double getRubnerBound(double[] rubnerValue, double[] cellRubner, int dimension) {
        if (dimension == 3) {
            double[] v1 = FormatUtils.getSubArray(cellRubner, 0, 3);
            ...
            if (x[0] && y[0] && z[0]) {
                ...
                return getLTWODist(FormatUtils.getSubArray(rubnerValue, 0, 2), FormatUtils.getSubArray(v1, 0, 2));
            }
            else if (x[0] && y[0] && z[2]) {
