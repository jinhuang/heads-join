...
public class ReductionBound implements Serializable {
    ...
    public double getReducedEmd(double[] histA, double[] histB) {
        ...
        return DistanceUtils.getEmd(reducedHistA, reducedHistB, 1, bins, DistanceType.ARBITRARY, getCostMatrix());
    }
    ...
    public static double[] getCostMatrixDoubleArray(int histLength,  double[] bins) {
        ...
        for (int i = 0; i < histLength; i++) {
            ...
            for (int j = 0; j < histLength; j++) {
                ...
                costMatrix[i * histLength + j] = DistanceUtils.getGroundDist(binsA, binsB, DistanceType.LTWO, null);
                binsB.clear();
            }
            binsA.clear();
        }
